...
describe("Captcha Component", () => {
  ...
  test("TC3 - Successful captcha input", async () => {
    ...
    fireEvent.change(screen.getByPlaceholderText(/Enter Captcha/i), { target: { value: "AAAAA" } });
    fireEvent.click(screen.getByRole("button", { name: /Submit/i }));
    await waitFor(() => {
      expect(screen.getByText((text) => text.includes("Captcha Verified"))).toBeInTheDocument();
    }, { timeout: 1000 });
  });
  ...
  test("TC4 - Incorrect captcha input", async () => {
    ...
    fireEvent.click(screen.getByRole("button", { name: /Submit/i }));
    expect(await screen.findByText(/Incorrect Captcha/)).toBeInTheDocument();
  });
